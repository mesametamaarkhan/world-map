...
private:
    ...
    //These rects are to determine countries/continents based on mouseclick on window
    //it uses the rectangle bounds to identify countries/continents
    //based on the marker user/player clicks on
...
public:
    ...
    //setting rectangles
    //these help identify location markers of countries/continents
    ...
    void drawText(const std::string& text, float x, float y, int fontSize = 30, const sf::Color& color = sf::Color::White) {
        sf::Font font;
        loadFont(font, "fonts/Poppins-Medium.ttf"); // Replace with the actual path to your font file
